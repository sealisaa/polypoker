...
//		testHandEvaluation(room, room.getPlayersMap().get("a"));
	...
	private static void testHandEvaluation(Room room, Player player) {
		...
		cardsList.add(player.getCard1());
		cardsList.add(player.getCard2());
		cardsList.add(room.getGameManager().getFaceUp().get(0));
		cardsList.add(room.getGameManager().getFaceUp().get(1));
		cardsList.add(room.getGameManager().getFaceUp().get(2));
		cardsList.add(room.getGameManager().getFaceUp().get(3));
		cardsList.add(room.getGameManager().getFaceUp().get(4));
