...
**1. Заход в комнату**
   ...
   2. После отправки сообщения клиентом, сервер принимает присланное сообщение, добавляет игрока к себе в комнату и отсылает клиенту ответное сообщение всем клиентам с типом `MessageType.PLAYER_ROOM_JOIN`
        ```
        {
            "messageType":"PLAYER_ROOM_JOIN",
            "content":
...
**4. Второй этап игры: Префлоп**
  ...
  10. Когда сервер в очередной раз будет проверять ставки игроков и в этот раз все поля `Player.currentStake` будут равны, сервер переводит комнату в следующий этап игры - изменяет этап на `GameState.FLOP`. Затем сервер отправляет всем клиентам сообщение с типом `MessageType.NEXT_STEP_OF_ROUND`
        ```
        {
            "messageType":"NEXT_STEP_OF_ROUND",
            "content":
        ...
        ```
  11. Клиент получает отправленное сообщение и изменяет этап игры в комнате на `GameState.FLOP`. После этого клиент проходится по списку игроков в комнате и складывает все значения полей `Player.currentStake`. Затем значение полученной суммы клиент помещает в поле `GameManager.bank`
...
**5. Третий этап игры: Флоп**
  ...
  8. Клиент получает отправленное сервером сообщение и
        ...
        - если `userLogin` не совпадает с логином текущего клиента то клиент ищет в списке игроков в комнате игрока по пришедшему логину и обновляет его поле `Player.currentStake`, и также обновляет пользовательский интерфейс - рядом с местом игрока должно обновится значение его текущей ставки
  9. Далее повторяются шаги с 4 по 8, пока ставки всех игроков не уравняются.
  10. Когда сервер в очередной раз будет проверять ставки игроков и в этот раз все поля `Player.currentStake` будут равны, сервер переводит комнату в следующий этап игры - изменяет этап на `GameState.TERN`. Затем сервер отправляет всем клиентам сообщение с типом `MessageType.NEXT_STEP_OF_ROUND`
        ```
        {
            "messageType":"NEXT_STEP_OF_ROUND",
            "content":
        ...
        ```
  11. Клиент получает отправленное сообщение и изменяет этап игры в комнате на `GameState.TERN`. После этого клиент проходится по списку игроков в комнате и складывает все значения полей `Player.currentStake`. Затем значение полученной суммы клиент помещает в поле `GameManager.bank`
  12. Переходим на следующий этап игры
...
**6. Четвертый этап игры: Тёрн**
  ...
  3. Клиент получает отправленное сообщение и изменяет этап игры в комнате на `GameState.RIVER`. После этого клиент проходится по списку игроков в комнате и складывает все значения полей `Player.currentStake`. Затем значение полученной суммы клиент помещает в поле `GameManager.bank`
...
**7. Пятый этап игры: Ривер**
  ...
  2. Повторяется алгоритм из 5ого пункта со 2ого шага по 10
  3. Клиент получает отправленное сообщение и изменяет этап игры в комнате на `GameState.SHOWDOWN`. После этого клиент проходится по списку игроков в комнате и складывает все значения полей `Player.currentStake`. Затем значение полученной суммы клиент помещает в поле `GameManager.bank`
  4.  Переходим на следующий этап игры
...
**8. Шестой этап игры: Шоудаун**
  ...
  2. Происходит вычисление победной комбинации. Клиент отправляет серверу сообщение с типом `MessageType.PAYMENT_TO_PLAYER`
        ```
        {
            "messageType":"PAYMENT_TO_PLAYER",
            "content":
                {
                    "roomCode": 1
                    "userLogin: "bh"
        ...
        ```
  3. Сервер получает отправленное сообщение и обновляет в Базе Данных для `User`, у которого значение поля `User.login` равно значению поля `userLogin` из присланного сообщения, следующие поля
  ...
  4. На клиенте появляется диалоговое окно с объявление победителя и размера выигрыша победителя. При нажатии кнопки `OK` в диалоговом окне клиент отправляет серверу сообщение с типом `MessageType.PLAYER_ROOM_EXIT`
        ```
        {
        ...
        ```
  5. Сервер получает отправленное клиентом сообщение и удаляет игрока из списка игроков в комнате
  6. Клиент покидает комнату и перемещается в Главное Меню.